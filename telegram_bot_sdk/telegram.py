...
class TelegramBot:
    def __init__(self, bool_async=True):
        self.config = load_config()
        self.my_network = network.NetworkInternalAsync(self.config) if bool_async else network.NetworkAsync(self.config)
    ...
    async def send_venue(self, *, chat_id, latitude, longitude, title, address, foursquare_id=None,
                         foursquare_type=None, disable_notification=None, reply_to_message_id=None, reply_markup=None):
        data_dict = check_locals(**locals())
    ...
    async def set_chat_title(self, chat_id):
        data_dict = check_locals(**locals())
    ...
    async def get_chat_members_counter(self, chat_id):
        data_dict = check_locals(**locals())
...
async def worker():
  ...
  #  task1 = asyncio.create_task(fetch_messages(t))
  #  await task1
